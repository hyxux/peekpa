...
def get_post_table(max_num):
    visitor_day_data = UserVisit.objects.filter(
        day=datetime.datetime.now().strftime('%Y-%m-%d'), end_point__contains='/detail/')
    post_map = {}
...
def get_dashboard_top_data():
    ...
    day_visit_num = DailyVisitNum.objects.filter(day=cur_date)[
        0].count if DailyVisitNum.objects.filter(day=cur_date).count() else 0
    total_visit_num = TotalVisitNum.objects.all(
    )[0].count if TotalVisitNum.objects.all().count() else 0
    context = {
        "post_num": post_num,
