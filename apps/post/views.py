...
@peekpa_tracking
def index(request):
    context = get_data_from_cache(
        request, get_index_data, redis_key="post:index")  # 读取缓存数据
    context.update(get_read_most_post())
    return render(request, 'post/index.html', context=context)


def get_index_data():
    top_post = Post.objects.filter(
        status=Post.STATUS_NORMAL, priority__gt=0).order_by('-priority')
    if len(top_post) > 4:
        top_post = top_post[:4]
...
def post_list_view(request):
    context = get_data_from_cache(
        request, get_post_list_data, redis_key="post:post_list")
    context.update(get_read_most_post())
    return render(request, 'post/list.html', context=context)


def get_post_list_data():
    hot_post = Post.objects.filter(
        status=Post.STATUS_NORMAL, is_hot=True).order_by('time_id')
    list_post = Post.objects.filter(
        status=Post.STATUS_NORMAL, is_hot=False).order_by('time_id')
    context = {
        'hot_post': hot_post,
        'list_post': list_post,
    }
...
def get_detail_data(time_id):
    post = Post.objects.select_related(
        'category', 'author').get(time_id=time_id)
...
def process_toc(toc):
    toc = toc.replace('<div class="toc">', '').replace('</div>', '').replace('<ul>',
                                                                             '<ul class="list-group">').replace('<li>', '<li class="border-0 list-group-item ">')
    if len(toc) <= 31:
        toc = None
    return toc
...
def get_advertise():
    advertise_link = Advertise.objects.all(
    )[0] if Advertise.objects.all().count() else None
    context = {
